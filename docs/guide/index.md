...
- **Workshop page** — `/static/companion/workshop/index.html`: browse the local catalog of published `.neko-companion` bundles (card grid + preview) and publish completed generation tasks.
- **Local model wizard** — `/static/companion/ollama/index.html`: probe Ollama, pick a model, and persist routing via `POST /api/companion/ai/open-source/config`.
...
- **Local open-source AI status** — `GET /api/companion/ai/open-source` probes the local Ollama daemon (`OLLAMA_HOST`, default `http://127.0.0.1:11434`) and returns availability plus the resolved model/base-URL routing config; when no key is configured the generation pipeline falls back to this route automatically (heuristics as last resort).
- **Corpus fact seeds (opt-in)** — set `extract_fact_seeds: true` in the generation input to add an LLM stage that extracts high-confidence facts from the corpus into the package manifest; on import they are written into the memory fact layer with `external_import` provenance. LLM-only by design: when no LLM is reachable the stage yields an empty list instead of fabricating facts (generation never fails because of it).
- **Persona refine and versions** — `POST /api/companion/persona/{name}/refine` runs one correction-tier LLM round over the existing character card and returns a diff proposal (nothing is written; `503` when the tier is not configured); `POST /api/companion/persona/{name}/refine/apply` persists a confirmed proposal after snapshotting the previous card into a version chain; `GET /api/companion/persona/{name}/versions` lists snapshots and `POST /api/companion/persona/{name}/rollback` restores one (the current card is snapshotted first, so rollbacks are revertible).
- **Sync protocol (read-only)** — `GET /api/companion/sync/manifest` returns a device-level snapshot (one `.neko-companion` manifest plus memory cursor per registered companion) and `GET /api/companion/sync/memory/{name}?since=...` serves idempotent, paginated fact-layer deltas for a second desktop instance (mobile clients later); desktop-authoritative, spec in `docs/companion-platform/SYNC_PROTOCOL.md`.
